...
0. @race HQ (with internet); estimated time 1h
  ...
  - foreach checkpoint pc X:
    ...
    - in screen, launch
        ...
        - `docker run --rm --add-host steenwerck.rfc1149.net:10.8.0.1 --name steenwerck-replicate -p 5984:5984 -v ~/steenwerck.conf:/steenwerck.conf -v steenwerck-data:/usr/local/var/lib/couchdb rfc1149/pointage100km replicate checkpoint X`
