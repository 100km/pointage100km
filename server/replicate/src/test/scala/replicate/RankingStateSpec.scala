...
class RankingStateSpec extends Specification {
  ...
  "RankingState" should {
    ...
    def first10(raceId: Int): Vector[Int] = {
    ...
    "get the right final order in race 1" in {
      first10(1) must be equalTo Vector(688, 24, 201, 719, 522, 242, 241, 110, 314, 217)
    }

    "get the right final order in race 2" in {
      first10(2) must be equalTo Vector(393, 491, 56, 490, 71, 91, 287, 670, 933, 560)
    }

    "get the right final order in race 3" in {
      first10(3) must be equalTo Vector(879, 867, 738, 498, 280, 735, 788, 790, 701, 759)
    }
  }
