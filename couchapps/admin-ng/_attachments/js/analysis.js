...
angular.module("admin-ng").controller("analysisPoint", ["$scope", "dbService",
    function($scope, dbService) {
      var setAction = function(label, icon, clazz, tooltip) {
      ...
      if ($scope.point.action === "add") {
        setAction("Add", "plus-sign", "success", "The algorithm suggests to add this point");
      } else if ($scope.point.action === "remove") {
        setAction("Remove", "trash", "danger", "The algorithm suggests to delete this point");
      } else if ($scope.point.type === "deleted") {
        setAction("Restore", "plus-sign", "deleted", "This time has been previously deleted");
      } else if ($scope.point.type === "down") {
        setAction();
      } else if ($scope.point.type === "artificial") {
        setAction("Remove", "remove", "artificial", "This time has been inserted manually");
      } else {
        setAction("Remove", "trash");
      }
